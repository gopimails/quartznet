...
namespace quartznet.jobs
{
    ...
    public class JobB : IJob
    {
        ...
        {
            _logger = logger;
            _logger.LogDebug("Constructed {job}", nameof(JobB));
        }
        public async Task Execute(IJobExecutionContext context)
        {
            context = context ?? throw new ArgumentNullException(nameof(context));

            _logger.LogInformation("{Job} Started", nameof(JobB));
            try
            {
            ...
            {
                _logger.LogError(ex, "{Job} Failed. \n Error : {Message} \n Skipping error.", nameof(JobB), ex.Message);
            }
            finally
            {
                _logger.LogDebug("{Job} Completed", nameof(JobB));
            }
        }
    }
}